...
function normaliseGitHubUrl(raw) {
    ...
    if (segments.length < 2)
        return fail("URL must include owner and repository (github.com/owner/repo).");
...
function validateName(raw, field, label) {
    const value = sanitise(raw);
    if (!value)
...
function validateInputs(repoUrl, teamName, leaderName) {
    const errors = [];
...
export function validateRunPayload(req, res, next) {
    ...
    const result = validateInputs(String(rawRepo ?? ""), String(rawTeam ?? ""), String(rawLeader ?? ""));
    if (!result.valid) {
        res.status(400).json({

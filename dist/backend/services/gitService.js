...
export const configureGitIdentity = async (repoPath) => {
    await execFileAsync("git", ["config", "user.name", "AI Healing Agent"], {
        cwd: repoPath,
        timeout: GIT_TIMEOUT,
    });
    await execFileAsync("git", ["config", "user.email", "ai-agent@users.noreply.github.com"], {
        cwd: repoPath,
        timeout: GIT_TIMEOUT,
    });
};
...
export const stageAndCommit = async (repoPath, message) => {
    ...
    const { stdout: nameOnly } = await execFileAsync("git", ["diff", "--cached", "--name-only"], {
        cwd: repoPath,
        timeout: GIT_TIMEOUT,
    });
...
export const pushBranch = async (repoPath, branchName, repoUrl, token) => {

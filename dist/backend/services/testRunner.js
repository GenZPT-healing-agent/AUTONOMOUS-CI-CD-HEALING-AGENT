...
import { isDockerAvailable, runInContainer, runSubprocess, } from "./containerSandbox.js";
/* ── Constants ── */
 ...
 */
export const detectProjectType = async (cloneDir) => {
    /* ── Node.js / TypeScript ── */
    ...
    if (pkgJson) {
        ...
        const hasRealTest = !!testScript && !testScript.includes("no test specified");
        ...
        const hasLockFile = await fileExists(path.join(cloneDir, "package-lock.json"));
        return {
            type: "node",
    ...
    /* ── Python ── */
    const hasRequirements = await fileExists(path.join(cloneDir, "requirements.txt"));
    ...
    if (hasPyproject || hasRequirements || hasSetupPy) {
        ...
        return {
            ...
            installCmd,
            testCmd: "python -m pytest --tb=short -q 2>&1 || python -m unittest discover -s tests 2>&1",
            buildCmd: "",
            hasTests: true,
        };
    }
    ...
    /* ── Java — Gradle ── */
    if ((await fileExists(path.join(cloneDir, "build.gradle"))) ||
 ...
 */
const parseTestFailures = (stdout, stderr, projectType) => {
    ...
    switch (projectType) {
        ...
        case "python": {
            ...
            if (pytestFails)
                failures.push(...pytestFails);
            const assertionErrors = combined.match(/^E\s+.+$/gm);
            if (assertionErrors)
                failures.push(...assertionErrors.slice(0, 10));
            break;
        }
        case "go": {
            const goFails = combined.match(/--- FAIL:\s+\S+/g);
            if (goFails)
                failures.push(...goFails);
            break;
        }
        case "java-maven":
        case "java-gradle": {
 ...
 */
const buildErrorSummary = (stdout, stderr, exitCode, timedOut) => {
    if (timedOut)
        return "Test execution timed out (exceeded 5-minute limit)";
    if (exitCode === 0)
 ...
 */
export const executeTests = async (cloneDir, config) => {
    // If we cannot determine project type or no commands exist, report honestly.
    if (!config.testCmd && !config.buildCmd) {
        console.warn(`[test-runner] No test or build command for project type "${config.type}" — skipping execution`);
        return {
            passed: false,
            exitCode: -1,
            stdout: "",
            stderr: "No test or build command detected for this project type. " +
                "The repository may lack a recognized build manifest (package.json, requirements.txt, go.mod, pom.xml, etc.).",
            durationMs: 0,
            failedTests: [],
            errorSummary: "Cannot execute tests — project type unrecognised or no test/build commands configured",
    ...
    if (useDocker && config.dockerImage) {
        console.log(`[test-runner] Docker execution — image: ${config.dockerImage}, cmd: ${command}`);
    ...
    const errorSummary = buildErrorSummary(raw.stdout, raw.stderr, raw.exitCode, raw.timedOut);
    const result = {
    ...
    console.log(`[test-runner] Result — passed: ${result.passed}, exit: ${result.exitCode}, ` +

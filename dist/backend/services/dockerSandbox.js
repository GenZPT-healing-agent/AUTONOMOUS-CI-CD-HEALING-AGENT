...
const extensionOf = (filePath) => path.extname(filePath).toLowerCase();
...
const detectIssuesInFile = async (absolutePath, repoRoot) => {
    ...
    const pushIssue = (bugType, lineNumber, fixSuggestion) => {
        if (issues.length >= 3) {
            return;
        }
        issues.push({
...
export const cloneRepository = async (repoUrl) => {
    ...
    try {
        ...
        throw new Error(`Repository clone failed: ${error instanceof Error ? error.message : "unknown error"}`);
    }
};
/* ── Scan an already-cloned repository for actionable issues ── */
export const scanForIssues = async (cloneDir) => {
    ...
    const dominantLanguage = [...languageBuckets.entries()].sort((a, b) => b[1] - a[1])[0]?.[0] ??
        "Unknown";
...
export const cleanupWorkspace = async (cloneDir) => {

...
const boot = async () => {
    try {
        ...
        console.error("[server] ⚠ Database connection failed at startup. " +
...
app.post("/api/sandbox/callback", async (req, res) => {
    ...
    await RunRepository.transitionStatus(runId, status ?? "completed", "Sandbox callback", { finishedAt: new Date().toISOString() });
    res.json({ ok: true });
});
/* ── Graceful shutdown ── */

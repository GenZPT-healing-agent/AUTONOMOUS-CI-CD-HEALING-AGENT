...
import type { RunResult } from "../types/agent.js";
...
const processRemediationJob = async (
  job: import("bullmq").Job<RemediationJobData>,
): Promise<void> => {
  const { runId, repoUrl, teamName, leaderName, retryLimit } = job.data;

  console.log(
    `[worker] ▸ Processing job ${job.id} — run=${runId} repo=${repoUrl}`,
  );
  ...
  try {
    ...
    console.log(
      `[worker] ✓ Run ${runId} completed — ci=${result.ciStatus}, ` +
...
remediationWorker.on("failed", (job, err) => {
  console.error(
    `[worker] Job ${job?.id ?? "unknown"} failed (attempt ${job?.attemptsMade ?? "?"}/${job?.opts?.attempts ?? "?"}): ${err.message}`,
  );
});
...
console.log(
  `[worker] Remediation worker started — queue="${REMEDIATION_QUEUE_NAME}", concurrency=2`,
);

...
export const dbLog = (entry: DbLogEntry): void => {
  const parts: string[] = [
    `[db]`,
...
export const initPool = async (): Promise<void> => {
  ...
  if (!connectionString) {
    throw new Error(
      'DATABASE_URL is not set. Provide a MongoDB Atlas connection string in your .env file.',
    );
  }
...
export const isPoolReady = (): boolean =>
  poolReady && mongoose.connection.readyState === 1;

/** Quick round-trip ping — used by the health-check route. */
export const pingDB = async (): Promise<{ ok: boolean; latencyMs: number }> => {
  const t0 = Date.now();
  try {

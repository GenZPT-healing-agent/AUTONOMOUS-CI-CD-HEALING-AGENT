...
const truncateOutput = (output) => {
    if (output.length <= MAX_OUTPUT_BYTES)
        return output;
    return (`…[truncated ${output.length - MAX_OUTPUT_BYTES} bytes]…\n` +
 ...
 */
const toDockerMountPath = (hostPath) => {
...
export const runInContainer = async (config) => {
    ...
    try {
        ...
        const e = err;
        const timedOut = e?.killed === true || durationMs >= config.timeoutMs - 1000;
        return {
...
export const runSubprocess = async (command, cwd, timeoutMs, env = {}) => {
    const startMs = Date.now();
...
//# sourceMappingURL=containerSandbox.js.map
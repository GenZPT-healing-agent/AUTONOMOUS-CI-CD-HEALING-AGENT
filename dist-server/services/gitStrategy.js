...
const githubApi = async (urlPath, opts) => {
 ...
 */
const checkWriteAccess = async (owner, repo, token) => {
    try {
 ...
 */
const forkRepository = async (owner, repo, token) => {
    try {
 ...
 */
const waitForFork = async (owner, repo, token, maxWaitMs = 60_000) => {
...
/* ── Pull Request Logic ── */
const createPullRequest = async (upstreamOwner, upstreamRepo, forkOwner, branchName, title, body, token) => {
    try {
...
export const pushWithStrategy = async (input) => {
    ...
    const authenticatedForkUrl = forkUrl.replace(/^https:\/\//, `https://x-access-token:${token}@`);
    try {
        ...
        const prUrl = await createPullRequest(owner, repo, username, branchName, commitTitle, commitBody, token);
        return {
            success: true,
            strategy: 'fork',
...
/* ── Direct Push Helper ── */
const attemptDirectPush = async (repoPath, branchName, repoUrl, token) => {
...
//# sourceMappingURL=gitStrategy.js.map
...
const resolvePipName = (importName) => {
...
const isNodeBuiltinModule = (name) => {
...
const addPythonDependency = async (clonePath, classification) => {
    ...
    const existingPkgs = new Set(content
        .split('\n')
...
const addNodeDependency = async (clonePath, classification) => {
...
const noOpStrategy = (category, strategy, reason) => ({
    applied: false,
    strategy,
    category,
    filesModified: [],
 ...
 */
export const executeRemediationStrategy = async (clonePath, classification, projectType) => {
    console.log(`[remediation-strategy] Executing strategy=${classification.suggestedStrategy} ` +
    ...
    switch (classification.suggestedStrategy) {
        ...
        case 'SETUP_ENVIRONMENT':
            return noOpStrategy(classification.category, 'SETUP_ENVIRONMENT', 'Environment/tool missing — cannot be fixed by code patches. Requires infrastructure change.');
        case 'ESCALATE':
            return noOpStrategy(classification.category, 'ESCALATE', 'Permission/access issue — cannot be fixed by the agent. Escalate to repo owner.');
        case 'SKIP':
            return noOpStrategy(classification.category, 'SKIP', 'No remediation needed (exit code 0 or no failure detected).');
        case 'STATIC_PATCH':
        default:
            return {
                applied: false,
...
//# sourceMappingURL=remediationStrategies.js.map
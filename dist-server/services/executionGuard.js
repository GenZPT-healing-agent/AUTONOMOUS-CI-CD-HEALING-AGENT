 ...
 * Returns size in MB.
 */
export const estimateRepoSize = async (repoPath) => {
    try {
        // Quick estimate: sum sizes of files in the root level only (fast heuristic)
        const { stdout } = await execFileAsync(process.platform === 'win32' ? 'cmd' : 'du', process.platform === 'win32'
            ? ['/c', `dir /s /a "${repoPath}" | findstr "File(s)"`]
            : ['-sm', repoPath], { timeout: 30_000 });
 ...
 */
export const validateRepoSize = async (repoPath, constraints = DEFAULT_CONSTRAINTS) => {
 ...
 */
export const trackProcess = (proc) => {
 ...
 */
export const killAllTrackedProcesses = () => {
    let killed = 0;
 ...
 * Returns RSS in MB.
 */
export const getCurrentMemoryMB = () => {
 ...
 */
export const validateMemory = (constraints = DEFAULT_CONSTRAINTS) => {
 ...
 */
export const canSkipInstall = async (runId, manifestPath) => {
 ...
 */
export const recordInstall = async (runId, manifestPath) => {
    const hash = await hashFile(manifestPath);
    if (hash) {
        const cacheKey = `${runId}:${manifestPath}`;
 ...
 */
export const clearInstallCache = (runId) => {

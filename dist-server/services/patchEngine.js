...
const computeLineFix = (line, bugType) => {
    switch (bugType) {
        case "LINTING": {
        ...
        case "IMPORT": {
            if (/^\s*from\s+.+\s+import\s+\*/.test(line) ||
...
const patchSingleFile = async (filePath, issues, clonePath) => {
...
export const applyAllPatches = async (issues, clonePath) => {

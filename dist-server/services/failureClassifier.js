...
const extractPythonModule = (match) => ({
    missingDependencies: match[1] ? [match[1].trim()] : [],
});
const extractNodeModule = (match) => ({
    missingDependencies: match[1] ? [match[1].trim().replace(/['"]/g, '')] : [],
});
const extractPipPackages = (_match, combined) => {
    const pkgs = [];
...
const extractFaultFilesFromTraceback = (_match, combined) => {
    const files = [];
...
export const classifyFailure = (input) => {
    ...
    for (const m of sameCategoryMatches) {
        if (m.extracted.missingDependencies) {
            for (const dep of m.extracted.missingDependencies)
                allDeps.add(dep);
        }
        if (m.extracted.faultFiles) {
            for (const f of m.extracted.faultFiles)
                allFiles.add(f);
        }
...
/* ── Helpers ── */
const buildUnknownClassification = (stderr, exitCode, combined) => {
    ...
    if (exitCode === 1)
        ...
        summary += ' — misuse of shell command';
    else if (exitCode === 126)
        summary += ' — command not executable';
    else if (exitCode === 127)
        summary += ' — command not found';
    else if (exitCode === 128)
        summary += ' — invalid exit argument';
    else if (exitCode > 128 && exitCode <= 192)
        summary += ` — killed by signal ${exitCode - 128}`;
    else if (exitCode === 137)
        summary += ' — killed by SIGKILL (OOM likely)';
...
const buildSummary = (category, deps, match, _combined, projectType) => {
    switch (category) {
 ...
 */
export const severityRank = (category) => {
    switch (category) {

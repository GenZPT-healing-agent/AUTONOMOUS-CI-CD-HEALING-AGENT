...
export const DEFAULT_PATCH_GUARD_CONFIG = {
 ...
 */
export const validatePatchImpact = async (repoPath, category, rationale, config = DEFAULT_PATCH_GUARD_CONFIG) => {
    ...
    try {
        ...
        const { stdout: nameOnly } = await execFileAsync('git', ['diff', '--cached', '--name-only'], { cwd: repoPath, timeout: 30_000 });
    ...
    let linesRemoved = 0;
    try {
        const { stdout: diffStat } = await execFileAsync('git', ['diff', '--cached', '--numstat'], { cwd: repoPath, timeout: 30_000 });
 ...
 */
export const rollbackToSnapshot = async (repoPath, snapshotSha) => {
 ...
 */
export const discardStagedChanges = async (repoPath) => {
 ...
 */
export const takeSnapshot = async (repoPath) => {
    try {
...
//# sourceMappingURL=patchGuard.js.map